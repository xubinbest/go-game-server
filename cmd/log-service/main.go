...
func main() {
	...
	if configLocation := config.GetConfigLocation(); configLocation == "local" {
		...
		content, err := reg.LoadConfig(ctx, "log.yaml", "DEFAULT_GROUP")
		if err != nil {
	...
	utils.Info("Starting log service", zap.Int("port", port))
...
func setupShutdownHandler(
	...
	go func() {
		utils.Info("Log gRPC service started", zap.Int("port", port))
