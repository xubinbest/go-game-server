...
│   ├── csv/             # CSV格式数据
│   └── xlsx/            # Excel格式数据
├── client/               # 客户端示例（支持HTTP和WebSocket）
├── scripts/              # 脚本文件（proto生成、K8s同步等）
├── sql/                  # 数据库初始化脚本
└── bin/                  # 启动脚本和编译输出
    └── scrpit/           # 本地开发启动脚本
```
...
**注意**：启动脚本位于 `bin/scrpit/` 目录（注意拼写），需要在项目根目录执行。

### Docker构建

#### 构建镜像
```bash
# 构建所有核心服务（gateway, social, user, leaderboard）
make build-all
...
**注意**：
- 镜像仓库地址在 `Makefile` 中配置，默认使用 `192.168.101.2:5000`
- `make build-all` 仅构建核心服务（gateway, social, user, leaderboard），不包括log-service
- 如需构建log-service，请单独执行 `make build-log-service`
...
- **启动脚本**: [bin/scrpit/](bin/scrpit/) - 本地开发启动脚本
  - `start_gateway.cmd` - 启动网关服务
  - `start_user.cmd` - 启动用户服务
  - `start_social.cmd` - 启动社交服务
...
- [客户端示例](client/README.md) - 客户端使用示例（支持HTTP和WebSocket）

## 🔄 更新日志

### v1.0.0 (当前版本)
- ✅ 初始版本发布
- ✅ 完整的微服务架构（6个核心服务：网关、用户、社交、游戏、排行榜、日志）
